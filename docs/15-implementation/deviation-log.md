...
| 2026-08-14 | API surface | Implemented REST surface diverges from `docs/09-api/rest-api.md`: market `quote/{symbol}` + `quotes` batch (spec: `quotes/{symbol}`); workflows flat `/workflows/{run_id}` (spec: `{workflow_id}/runs` hierarchy); rpc commands `run-decision-cycle`/`halt-trading`/`resume-trading`/`cancel-order` (spec: `trigger-workflow`/`kill-switch`/`override-proposal`); kill-switch pindah ke `/admin/kill-switch` (dengan tier); pagination offset/limit (spec: cursor-based) | Implementasi mengikuti frontend client contracts (`frontend/src/lib/api/`) yang sudah teruji; `docs/09-api/openapi.yaml` (regenerated byte-identical dari code) = machine-readable source of truth; tabel divergensi lengkap di section "Implemented Surface Reconciliation" `docs/09-api/rest-api.md` | ADR (pending) | — |
