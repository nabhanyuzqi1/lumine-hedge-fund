...
- **Progress 2026-08-14 (final):** B-01..B-07 CLOSED (679 backend tests) · W2 F-01..F-12 CLOSED (172 frontend tests) · W3 docker CLOSED · W4 LIVE https://lumine.biz.id (Cloudflare Flexible SSL, domain aktif) · **Keycloak 26.0.8 LIVE** (ganti Authelia 4.38 — Mixed Content fixed, admin console accessible) · G-01 CLOSED (EMERGENCY-RUNBOOK.md 11 seksi) · G-06 CLOSED (backup-postgres+health-check+resource-watchdog cron) · G-08 CLOSED (.github/workflows/ci.yml + GitHub secrets VPS_HOST+VPS_SSH_KEY) · **G-07 CLOSED** (DNS router.lumine.biz.id accessible) · B-05 PARTIAL CLOSED (DEMO_DATA=false, orders+positions DB-backed) · Landing page publik (Linear design, /login, role-based routing user/admin/superadmin, logout TopBar) · Bloomberg Terminal UI (superadmin live clock + compact mono tabs) · Sisa: LLM_GATEWAY_API_KEY (provider mengisi), login akun HFM via noVNC, seed produksi, B-08 backfill, runner CI Ubuntu.
...
|| G-09 | Keycloak SSO — **CLOSED 2026-08-14** | Keycloak 26.0.8 replace Authelia 4.38 (Mixed Content fixed: KC_HOSTNAME_ADMIN_URL + KC_HOSTNAME_STRICT=false + KC_HTTP_ENABLED=true); admin console `https://lumine.biz.id/auth/admin/` accessible (superadmin/Lumine@2026!) | ✅ |
|| G-10 | Logout UI — **CLOSED 2026-08-14** | TopBar logout button + username display di terminal; `useAuth()` + `handleLogout()` → localStorage clear + redirect `/login`; tests 172/172 pass | ✅ |
