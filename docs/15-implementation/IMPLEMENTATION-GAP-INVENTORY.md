...
- **Progress 2026-08-14 (final):** B-01..B-07 CLOSED (679 backend tests) · W2 F-01..F-12 CLOSED (172 frontend tests) · W3 docker CLOSED · W4 LIVE https://lumine.biz.id (Cloudflare Flexible SSL, domain aktif) · G-01 CLOSED (EMERGENCY-RUNBOOK.md 11 seksi) · G-06 CLOSED (backup-postgres+health-check+resource-watchdog cron) · G-08 CLOSED (.github/workflows/ci.yml + GitHub secrets VPS_HOST+VPS_SSH_KEY) · G-07 CLOSED (DNS router.lumine.biz.id accessible) · B-05 PARTIAL CLOSED (DEMO_DATA=false, orders+positions DB-backed) · Landing page publik (Linear design, /login, role-based routing user/admin/superadmin, logout TopBar) · Bloomberg Terminal UI (superadmin live clock + compact mono tabs) · **AUTH-01 CLOSED — internal session auth menggantikan Authelia + Keycloak (keduanya dihapus dari VPS)**: users table (migrasi 0012), PBKDF2-HMAC-SHA256, cookie session HMAC-signed, Caddy forward_auth → `/api/auth/verify` (role-gated), backend 698 pass · Landing page rebuilt shadcn/ui + Linear design system · Terminal rebuilt (SSE live, zero demo streams) · Superadmin rebuilt (zero demo fallback, PUT config real) · Sisa: LLM_GATEWAY_API_KEY (provider mengisi), login akun HFM via noVNC, seed produksi, B-08 backfill, runner CI Ubuntu.
