...
async def _backfill_fills(session, ids: dict[str, uuid.UUID]) -> int:
    ...
    for order in orders:
        ...
        lineage = LineageRecord(
            ...
            model_version_ids={"technical_analyst": str(ids["model"])},
            prompt_version_ids={
                "technical_analyst": str(ids["prompt:technical_analyst"]),
                "macro_analyst": str(ids["prompt:macro_analyst"]),
                "risk_officer": str(ids["prompt:risk_officer"]),
            },
